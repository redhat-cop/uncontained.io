---
date: "2018-02-24T01:07:30-05:00"
...
The Ansible config playbook will by default generate a bunch of certificates that will be used by various components. If you have need to customize these certificates, consult link:/guides/openshift-custom-certificates[Configuring OpenShift to use Custom Certificates].
...
See our link:/guides/openshift-load-balancing[Integrating External Loadbalancers] guide for more details on this.
...
----
openshift_node_kubelet_args="{'kube-reserved': ['cpu=250m,memory=500M'], <1>
  'system-reserved': ['cpu=250m,memory=500M'], <2>
  'eviction-hard': ['memory.available<100Mi'], <3>
  'minimum-container-ttl-duration': ['10s'], <4>
  'maximum-dead-containers-per-container': ['2'], <5>
  'maximum-dead-containers': ['50'], <6>
  'pods-per-core': ['10'], <7>
  'max-pods': ['250'], <8>
  'image-gc-high-threshold': ['80'], <9>
  'image-gc-low-threshold': ['60'] <10>
}"
----
...
Once we have everything prepped, it's a good idea to run through our link:/guides/openshift-pre-install[OpenShift Pre-Install Validation Checklist].
...
link:/guides/openshift-validate[Validating an OpenShift Install]

== What's Next?

* link:/guides/openshift-admin-overview[Overview of OpenShift for Administrators]
* link:/guides/openshift-custom-certificates[Configuring OpenShift to use Custom Certificates]
* link:/guides/openshift-ldap-integration[Adding LDAP for Authentication]
