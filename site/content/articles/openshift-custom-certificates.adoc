...
_This is a followup to link:/articles/openshift-ha-installation[Installing a Highly Available OpenShift Cluster]. Many assumptions are made based on the information in that guide. If you have not yet been through this guide, we recommend doing so before continuing._
...
IMPORTANT: In order to do the above, master cluster public hostname and master cluster hostname  must be different.  If they’re the same, the named certificates will fail and will require a re-install. See the link:/articles/openshift-ha-installation/#_dns[Designing for DNS] section of the Install Guide.
...
For other components like Logging & Metrics, the approach for fronting services with custom certs can be found in link:{docs_url}install_config/certificate_customization.html[Administrator Solutions for Certificate Management].
...
* link:{docs_url}install_config/certificate_customization.html[Official Docs on Certificate Management for Services]
