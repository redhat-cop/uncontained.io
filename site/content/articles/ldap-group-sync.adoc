---
title: LDAP Group Synchronization with OpenShift
date: 2017-06-10T00:24:36-04:00
authors: ["redhat-cop"]
categories:
  - 'Add On'
tags:
...
This guide discusses the synchronization of groups defined in an LDAP server with OpenShift and is distinct from using an LDAP server to authenticate users to OpenShift. Please refer to the link:/articles/openshift-ldap-integration/[LDAP Integration guide] for using an LDAP server as an identity provider to govern user authentication to OpenShift.
...
NOTE: In the link:/articles/openshift-ldap-integration/[LDAP Integration for Authentication guide], an link:/articles/openshift-ldap-integration/#_example_ldap_tree_structure[example LDAP tree structure] was provided as a use case and will be used during this discussion to synchronize groups from LDAP to OpenShift.
