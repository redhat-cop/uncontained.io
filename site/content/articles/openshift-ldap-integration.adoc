---
date: "2018-02-24T01:28:04-05:00"
title: "Openshift LDAP Integration"
authors: ["sabre1041", "jaredburck"]
categories:
  - 'Add On'
tags:
  - openshift
...
The Ansible based OpenShift installer contains functionality to automate the configuration of the `LDAPPasswordIdentityProvider`. Several link:{docs_url}install/configuring_inventory_file.html#configuring-cluster-variables[cluster variables] of the Ansible link:{docs_url}install/configuring_inventory_file.html#configuring-ansible[inventory] can be specified to tune the configuration of identity providers. For LDAP, these include:
...
This guide focused on authenticating users into OpenShift against an LDAP server. To synchronize groups defined within the LDAP server into OpenShift, please refer to the link:../ldap-group-sync/[LDAP Group Synchronization guide].
