---
date: "2016-08-18T01:07:30-05:00"
...
The `openshift_hosted_router_selector` and `openshift_hosted_registry_selector` Ansible settings determine the label selectors used when placing registry and router pods. They are set to `region=infra` by default:
...
----
<1>  kubeletArguments.kube-reserved = Resources reserved for node components
<2>  kubeletArguments.system-reserved = Resources reserved for the remaining system components
<3>  kubeletArguments.maximum-dead-containers-per-container = The number of instances to retain per pod container
<4>  kubeletArguments.maximum-dead-containers = node removes the dead containers, all files inside those containers are removed as well
<5>  kubeletArguments.pods-per-core = How many pods will be allowed to run per core
<6>  kubeletArguments.max-pods = Max number of pods on a node
<7>  kubeletArguments.image-gc-high-threshold = The percent of disk usage which triggers image garbage collection
<8>  kubeletArguments.image-gc-low-threshold = The percent of disk usage to which image garbage collection attempts to free
